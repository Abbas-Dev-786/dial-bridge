...
export function CampaignHeader() {
  ...
  const campaign = activeCampaign || {
    name: isLoading ? `Loading...` : `Unknown`,
    status: "draft",
  };
  const campaignStatus = activeCampaign?.status || "draft";
  const isDraft = campaignStatus === "draft";
  const availableTransitions =
    STATUS_TRANSITIONS[campaignStatus as keyof typeof STATUS_TRANSITIONS] || [];
    ...
    transitionStatus.mutate(target, {
      onSuccess: () => {
        toast({
          ...
          description: `Campaign is now ${target}.`,
        });
      },
      onError: () => {
        toast({
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => setDialogState("export", true)}
          >
